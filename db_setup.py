import os

import numpy as np
import sqlite3

import ollama
import sqlite_vec


def get_embed_blob(text: str) -> bytes:
    response = ollama.embeddings(
        model="nomic-embed-text",
        prompt="search_query: " + text.lower().strip()
    )
    vec = np.array(response["embedding"], dtype=np.float32)
    vec /= np.linalg.norm(vec)
    return vec.tobytes()


def embed_and_store(conn, text: str):
    cursor = conn.cursor()
    if not cursor.execute("SELECT 1 FROM item_embeds WHERE name = ?", (text,)).fetchone():
        blob = get_embed_blob(text)
        cursor.execute("INSERT INTO item_embeds(name, embed) VALUES (?, ?)", (text, blob))
    conn.commit()

def load_sqlite_vec(conn: sqlite3.Connection):
    conn.enable_load_extension(True)
    sqlite_vec.load(conn)
    conn.enable_load_extension(False)


def create_tables():
    with open('data.csv', mode='r', encoding='utf-8-sig') as f:
        elems = [x for line in f.readlines() for x in line.strip('\n').split(',') if x]

    with sqlite3.connect('combinations.db') as conn:
        load_sqlite_vec(conn)

        cursor = conn.cursor()
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS items (
                id INTEGER PRIMARY KEY,
                name TEXT UNIQUE NOT NULL,
                is_base INTEGER NOT NULL DEFAULT FALSE,
                is_goal INTEGER NOT NULL DEFAULT FALSE
            );
        """)
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS recipe (
                item1_id INTEGER NOT NULL,
                item2_id INTEGER NOT NULL CHECK (item2_id >= item1_id),
                result_id INTEGER,
                desc TEXT,
                reasoning TEXT,
                FOREIGN KEY (item1_id) REFERENCES items(id)
                FOREIGN KEY (item2_id) REFERENCES items(id)
                FOREIGN KEY (result_id) REFERENCES items(id)
                PRIMARY KEY (item1_id, item2_id)
            );
        """)
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS candidate_recipe (
                item1_id INTEGER NOT NULL,
                item2_id INTEGER NOT NULL CHECK (item2_id >= item1_id),
                result_id INTEGER,
                desc TEXT,
                reasoning TEXT,
                rank INTEGER NOT NULL,
                status TEXT DEFAULT 'candidate',
                FOREIGN KEY (item1_id) REFERENCES items(id)
                FOREIGN KEY (item2_id) REFERENCES items(id)
                FOREIGN KEY (result_id) REFERENCES items(id)
                PRIMARY KEY (item1_id, item2_id, rank)
            );
        """)
        conn.execute("""
            CREATE VIRTUAL TABLE IF NOT EXISTS item_embeds USING vec0(
                name TEXT UNIQUE,
                embed FLOAT[768] distance_metric=cosine
            );
        """)

        cursor.executemany("INSERT OR IGNORE INTO items (name) VALUES (?)", [(x,) for x in elems])
        for elem in elems:
            embed_and_store(conn, elem)


if not os.path.exists("combinations.db"):
    create_tables()
